...
from utils.loaddata import load_batch_level_dataset, load_entity_level_dataset, load_metadata,\
    darpa_load_metadata,darpa_load_entity_level_dataset,\
    atlas_load_metadata,atlas_load_entity_level_dataset
...
def main(main_args):
    ...
    if dataset_name == 'streamspot' or dataset_name == 'wget':
        ...
        metadata = atlas_load_metadata(dataset_name)
        main_args.n_dim = metadata['node_feature_dim']
        main_args.e_dim = metadata['edge_feature_dim']
        model = build_model(main_args)
        safe_name = dataset_name.replace("/", "_")
        model.load_state_dict(torch.load("./checkpoints/checkpoint-{}.pt".format(safe_name), map_location=device))
        model = model.to(device)
        model.eval()
        ...
        with torch.no_grad():
            ...
            for i in range(n_train):
                g = atlas_load_entity_level_dataset(dataset_name, 'train', i).to(device)
            ...
            for i in range(n_test):
                g = atlas_load_entity_level_dataset(dataset_name, 'test', i).to(device)
            ...
            del x_test, y_test
            test_auc, test_std, _, _ = evaluate_entity_level_using_knn(safe_name, x_train, result_x_test,
                                                                       result_y_test)
