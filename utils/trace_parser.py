...
metadata = {
    ...
    'theia/theia311':{
            'train': ['ta1-theia-e3-official-6r.bin.7.json', 'ta1-theia-e3-official-6r.bin.9.json'],
            'test': ['ta1-theia-e3-official-6r.bin.8.json']
    },
    'cadets':{
            'train': ['ta1-cadets-e3-official.json','ta1-cadets-e3-official.json.1', 'ta1-cadets-e3-official.json.2', 'ta1-cadets-e3-official-2.json.1'],
            'test': ['ta1-cadets-e3-official-2.json']
...
def read_single_graph(dataset, malicious, path, test=False):
    ...
    path = '../data_files/{}/'.format(dataset) + path + '.txt'
    f = open(path, 'r')
    lines = []
...
def preprocess_dataset(dataset):
    ...
    for key in metadata[dataset]:
        for file in metadata[dataset][key]:
            if os.path.exists('../data_files/{}/'.format(dataset) + file + '.txt'):
                continue
            f = open('../data_files/{}/'.format(dataset) + file, 'r', encoding='utf-8')
            fw = open('../data_files/{}/'.format(dataset) + file + '.txt', 'w', encoding='utf-8')
    ...
    if len(id_nodename_map) != 0:
        fw = open('../data_files/{}/'.format(dataset) + 'names.json', 'w', encoding='utf-8')
        json.dump(id_nodename_map, fw)
    if len(id_nodetype_map) != 0:
        fw = open('../data_files/{}/'.format(dataset) + 'types.json', 'w', encoding='utf-8')
        json.dump(id_nodetype_map, fw)


def read_graphs(dataset):
    malicious_entities = '../data_files/{}/label.txt'.format(dataset, dataset)
    ...
    if os.path.exists('../data_files/{}/names.json'.format(dataset)) and os.path.exists('../data/{}/types.json'.format(dataset)):
        with open('../data_files/{}/names.json'.format(dataset), 'r', encoding='utf-8') as f:
            id_nodename_map = json.load(f)
        with open('../data_files/{}/types.json'.format(dataset), 'r', encoding='utf-8') as f:
            id_nodetype_map = json.load(f)
        f = open('../data_files/{}/malicious_names.txt'.format(dataset), 'w', encoding='utf-8')
    ...
    pkl.dump((final_malicious_entities, malicious_names), open('../data_files/{}/malicious.pkl'.format(dataset), 'wb'))
    pkl.dump([nx.node_link_data(train_g) for train_g in train_gs], open('../data_files/{}/train.pkl'.format(dataset), 'wb'))
    pkl.dump([nx.node_link_data(test_g) for test_g in test_gs], open('../data_files/{}/test.pkl'.format(dataset), 'wb'))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CDM Parser')
    parser.add_argument("--dataset", type=str, default="theia/theia311")
    args = parser.parse_args()
    if args.dataset not in ['trace', 'theia/theia311', 'cadets']:
